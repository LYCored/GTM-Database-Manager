...
oSelectList.onclick = function(e, a) {
    ...
    if (isCusDate) {
        radioVal41 = oCustext3.value;
        radioVal42 = oCustext4.value;
    } else {
        radioVal41 = '';
        radioVal42 = '';
    }
    ...
    if (tagName == 'INPUT') {
        ...
        else if (tag.getAttribute('name').toUpperCase() == 'DATEBUTTON') { //如果点击的是 自定义日期按钮
            radioVal41 = oCustext3.value;
            radioVal42 = oCustext4.value;
            isCusDate = true;
            dateFiltrate();
    ...
    if (tagName == 'A') { //如果点击 的是 A
        ...
        if (!oPrevInput) { //如果上一个节点没有则认为点击的是 '不限'
            ...
            if (trim(prev(parent).innerHTML) == '商品售价') { //这里是直接根据 text来比较的.建议加个自定义属性作标识符
                for (var i = 0; i < aRadio2.length; i++) {
                    aRadio2[i].checked = false;
                }
                isCusPrice = false;
            ...
            else if (trim(prev(parent).innerHTML) == '售货员姓名'){
            ...
            else if (trim(prev(parent).innerHTML) == '出售日期'){
    ...
    /*for (var i = 0; i < aRadio4.length; i++) {
        if (aRadio4[i].checked) {
            radioVal41 = next(aRadio4[i]).innerHTML;
            isCusDate = false;
            break;
        }
    }*/
    ...
    if(a && a == 2) {
        ...
        if (radioVal2){
            infor += '<div class=\"selectedInfor selectedShow\"><span>商品售价:</span><label>' + radioVal2 + '</label><em p="2"></em></div>';
            priceFiltrate();
        }
        if (radioVal3){
            infor += '<div class=\"selectedInfor selectedShow\"><span>售货员姓名:</span><label>' + radioVal3 + '</label><em p="2"></em></div>';
            nameFiltrate();
        }
        if (radioVal41 && radioVal42){
            infor += '<div class=\"selectedInfor selectedShow\"><span>出售日期:</span><label>' + radioVal41 + '至' + radioVal42 + '</label><em p="2"></em></div>';
            dateFiltrate();
        }
    }
...
function dateFiltrate() {
    $.post("filtrate.php",
        {
            valueType: "sRecordDate",
            value1: radioVal41,
            value2: radioVal42
        },
        function (data, status) {
...
function sRecordClearAll() {
    $.post("filtrate.php",
        {
            valueType: "sRecordClearAll",
        },
        function (data, status) {
            if (!status){
                alert("Error: post is fail!")
