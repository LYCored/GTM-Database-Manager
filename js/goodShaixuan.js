...
oSelectList.onclick = function(e, a) {
    ...
    if (tagName == 'A') { //如果点击 的是 A
        ...
        if (!oPrevInput) { //如果上一个节点没有则认为点击的是 '不限'
            ...
            if (trim(prev(parent).innerHTML) == '商品价格') { //这里是直接根据 text来比较的.建议加个自定义属性作标识符
                for (var i = 0; i < aRadio2.length; i++) {
                    aRadio2[i].checked = false;
                }
                isCusPrice = false;
            ...
            else if (trim(prev(parent).innerHTML) == '商品品牌'){
            ...
            else if (trim(prev(parent).innerHTML) == '商品种类'){
    ...
    if(a && a == 2) {
        ...
        if (radioVal2){
            infor += '<div class=\"selectedInfor selectedShow\"><span>商品价格:</span><label>' + radioVal2 + '</label><em p="2"></em></div>';
            priceFiltrate();
        }
        if (radioVal3){
            infor += '<div class=\"selectedInfor selectedShow\"><span>商品品牌:</span><label>' + radioVal3 + '</label><em p="2"></em></div>';
            brandFiltrate();
        }
        if (radioVal4){
            infor += '<div class=\"selectedInfor selectedShow\"><span>商品种类:</span><label>' + radioVal4 + '</label><em p="2"></em></div>';
            kindFiltrate();
        }
    }
...
function goodClearAll() {
    $.post("filtrate.php",
        {
            valueType: "goodClearAll",
        },
        function (data, status) {
            if (!status){
                alert("Error: post is fail!")
