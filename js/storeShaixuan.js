...
oSelectList.onclick = function(e, a) {
    ...
    if (tagName == 'A') { //如果点击 的是 A
        ...
        if (!oPrevInput) { //如果上一个节点没有则认为点击的是 '不限'
            ...
            if (trim(prev(parent).innerHTML) == '店铺地址') { //这里是直接根据 text来比较的.建议加个自定义属性作标识符
    ...
    if(a && a == 2) {
        ...
        if (radioVal2){
            infor += '<div class=\"selectedInfor selectedShow\"><span>店铺地址:</span><label>' + radioVal2 + '</label><em p="2"></em></div>';
            addressFiltrate();
        }
    }
...
function addressFiltrate() {
    $.post("filtrate.php",
        {
            valueType: "storeAddress",
            value: radioVal2
        },
